    ...
    with SessionLocal() as db:
        db.add(
            AIInteractionMetric(
                ...
                input_tokens=ai_output.response.input_tokens,
                output_tokens=ai_output.response.output_tokens,
...
def create_feedback(conversation_id: str, user_id: str, rating: int, comment: str | None = None) -> dict:
    with SessionLocal() as db:
        ...
        item = ConversationFeedback(conversation_id=conversation_id, user_id=user_id, rating=rating, comment=comment)
        db.add(item)
        db.commit()
        db.refresh(item)
...
def quality_summary() -> dict:
    with SessionLocal() as db:
        ...
        input_tokens = db.scalar(select(func.sum(AIInteractionMetric.input_tokens))) or 0
        output_tokens = db.scalar(select(func.sum(AIInteractionMetric.output_tokens))) or 0
